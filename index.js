...
var reactNativePackageJSONPath = function () {
    return path.resolve(
        process.cwd(),
        'node_modules',
        'html-native',
...
if (cli) {
    ...
    if (commands.length === 0) {
        console.error(
            'You did not pass any commands, did you mean to run `html-native init`?'
        );
        process.exit(1);
    }
    ...
    switch (commands[0]) {
        case 'init':
            if (!commands[1]) {
                console.error(
                    'Usage: html-native init <ProjectName> [--verbose]'
                );
                process.exit(1);
            } else {
                if (!argv.verbose) {
        ...
        default:
            console.error(
                ...
                + 'Make sure that you have run `npm install` and that you are inside a html-native project.',
                commands[0]
            );
            process.exit(1);
            break;
...
function createProject(name, verbose, rnPackage) {
    ...
    console.log(
        'This will walk you through creating a new html Native project in',
        root
    );
    ...
    var packageJson = {
        ...
        private: true,
        scripts: {
            start: 'node node_modules/html-native/local-cli/cli.js start'
        }
    };
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify(packageJson));
    process.chdir(root);

    console.log('Installing html-native package from npm...');
...
function getInstallPackage(rnPackage) {
    var packageToInstall = 'html-native';
...
function run(root, projectName, rnPackage) {
    exec('npm install --save --save-exact ' + getInstallPackage(rnPackage), function (e, stdout, stderr) {
        if (e) {
            ...
            console.error('`npm install --save --save-exact html-native` failed');
            process.exit(1);
        }
...
function runVerbose(root, projectName, rnPackage) {
    var proc = spawn(/^win/.test(process.platform) ? 'npm.cmd'
        : 'npm', ['install', '--verbose', '--save', '--save-exact', getInstallPackage(rnPackage)], {stdio: 'inherit'});
...
function checkForVersionArgument() {
    if (argv._.length === 0 && (argv.v || argv.version)) {
        console.log('html-native-cli: ' + require('./package.json').version);
        try {
            console.log('html-native: ' + require(reactNativePackageJSONPath()).version);
        } catch (e) {
